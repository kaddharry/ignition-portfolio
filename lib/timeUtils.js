...
function parseNaturalSlot(text) {
    ...
    // Reject obviously past/invalid relative words before anything else
    const lowerCheck = text.toLowerCase().trim();
    if (
        lowerCheck.startsWith('yesterday') ||
        lowerCheck.includes('last week') ||
        lowerCheck.includes('last month') ||
        lowerCheck.includes('last year')
    ) {
        return null; // caller should show 'past date' message
    }
...
function isWeekday(eventStart) {
    // Use IST midday (T12:00:00+05:30) as anchor so getDay() returns the IST day,
    // not the UTC day which can be 1 behind due to +05:30 offset.
    const istDateStr = eventStart.toLocaleDateString('en-CA', { timeZone: TZ });
    const istNoon    = new Date(istDateStr + 'T12:00:00+05:30');
    const dow = istNoon.getDay(); // 0=Sun, 6=Sat
    return dow !== 0 && dow !== 6;
}
